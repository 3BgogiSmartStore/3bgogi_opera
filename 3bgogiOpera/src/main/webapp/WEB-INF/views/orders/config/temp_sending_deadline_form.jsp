        ...
        <div class="splash-container">
        	<div class="card">        	
	            <div class="card-header" style="text-align: center;">
	                <h4 class="mb-1" style="color:red;"> 임시 발송일 지정하기 </h4>
	                
	            </div>
	            <form class="card-body" id="tempsendingDeadlineForm" method="POST">
	            	...
	            	<div class="form-group">
		                <label for="aaWorkStart"> 임시 발송일 </label>
		                <input class="form-control" id="dateStart" name="dateStart" type="text" value="${osVO.dateStart }">
	                </div>
	            	<div class="form-group">
	            		<button type="submit" class="btn btn-primary btn-block"> 지정하기 </button>
	            	</div>
	            </form>
        	</div>
        </div>
...
<script type="text/javascript">
		$(function() {
			...
			$("#tempsendingDeadlineForm").submit(function(){
				var orSendingDeadline = $("#dateStart").val();
				
				if(confirm(orSendingDeadline+" 로 임시 발송 날짜를 지정하시겠습니까?")){					
					
					var sendingDeadlineChangeFormData = jQuery("#tempsendingDeadlineForm").serialize();
					
					$.ajax({
						type : "POST",
						url:"<c:url value='/order/config/add/temp_sending_deadline.do'/>",
